...
const TerrainTreader = async (db: Db, data: { [key: string]: string }, res: Response) => {
  try {
...
const RoboSoccer = async (db: Db, data: { [key: string]: string }, res: Response) => {
  try {
...
async function is_player_id_present(pid: string, collection: Collection) {
...
const register_bgmi = async (req: CustomRequest, res: Response) => {
  const db = req.db!;
...
const Aerofilia = async (db: Db, data: { [key: string]: string }, res: Response) => {
  try {
    data.Team_key = data.Team_name.toUpperCase();
...
const LogoDesign = async (db: Db, data: { [key: string]: string }, res: Response) => {
...
const Circuitrix = async (db: Db, data: { [key: string]: string }, res: Response) => {
  console.log(data);
...
const Valorant = async (db: Db, data: { [key: string]: string }, req: CustomRequest, res: Response) => {
  data.Team_key = data.Team_name.toUpperCase();
  const file = req.file;
  delete data.file;
...
async function check_number(number: string, collection: Collection) {
  const c1 = await collection.findOne({ Leader_whatsapp: number });
  const c2 = await collection.findOne({ P2_number: number });
  const c3 = await collection.findOne({ P3_number: number });
...
const Autocad = async (db: Db, data: { [key: string]: string }, res: Response) => {
  data.Team_key = data.Team_name.toUpperCase();
